...
(* | (FuryVar x) -> (try (lookupobject x) with LookupError -> raise UnboundVariableError) *)
  ...
  | (FuryNegate (e1)) -> let e1' = (evalnoenv e1) in (FuryNegate(e1'))
